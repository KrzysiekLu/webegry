...
const contact = ()=>{
    ...
    const checkName = ()=>{
        ...
        return name.length > 0 ? (hideError(inputName), true) : (errorMessage = "Name field is empty", displayError(inputName, errorMessage), inputName.classList.add("input-error"));
    };
    // validate phone
    ...
    function checkPhone(e) {
        ...
        if (phoneInput.isValidNumber(e)) return true;
        else return false;
    }
    // validate Email
    const checkEmail = (mail)=>{
        const mailFormat = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
        if (inputEmail.value.match(mailFormat)) return true;
        return false;
    };
    ...
    const validate = ()=>{
        if (checkName() && checkContact() && checkMessage()) return true;
        else {
            checkContact();
            checkMessage();
            return false;
